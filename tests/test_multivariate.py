"""Tests for exuber.multivariate. The PCA formula-exact check and the
N-dependence sanity check mirror docs/replication/multivariate/
radf_common_validation.py (same checks, re-implemented here rather than
imported -- that script lives in the umbrella repo, one level up from
pyexuber's own repo boundary, so pyexuber's CI (which only checks out
`kvasilopoulos/pyexuber`) can't reach it; see that script's module
docstring for the R reference numbers this cross-checks against and full
narrative)."""
...
from exuber.multivariate import RadfCommonCv, RadfCommonResult, _pca, radf_common, radf_common_cv
