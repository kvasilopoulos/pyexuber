...
validation record):
  ...
  - dating_hls(): Harvey, Leybourne & Sollis (2017) SSR+BIC dating --
    ...
    BIC model selection.
  - dating_hlw(): Harvey, Leybourne & Whitehouse (2020) two-step
...
from exuber.cv import PCNT, radf_wb_cv
from exuber.datestamp import SIG_IDX, Episode, datestamp
...
# _hls_fit_series() independently within each window -- restricted to
# models {2, 4} for every window but the last (a window boundary is by
# construction a unit-root point, not a genuine sample end) -- with a
# sequential adjustment rule so window j+1 always starts at the first
# observation of window j's just-fitted post-explosive regime.
#
...
def _dating_hlw_from_episodes(
    ...
    _recovery_dates_from_bsadf() for radf_recovery()).
    """
    nhat = len(episodes)
    if nhat == 0:
        return []
